...
std::string removeAllMarkups(const std::string& str)
{
    std::array<std::regex, 7> links =
    {
        std::regex(R"(\[\[([^\]]*)\]\])")
      , std::regex(R"(\[\[([^\]]*)\|([^\]]*)\]\])")
      , std::regex(R"(\[\[([\]]*)#([^\]]*)\|([^\]]*)\]\])")
    ...
    std::string ret = str;
    if (std::regex_search(ret, links[2])) {
        ret = std::move(std::regex_replace(ret, links[2], "$3"));
    }

    if (std::regex_search(ret, links[1])) {
        ret = std::move(std::regex_replace(ret, links[1], "$2"));
    }

    if (std::regex_search(ret, links[0])) {
        ret = std::move(std::regex_replace(ret, links[0], "$1"));
    }

    if (std::regex_search(ret, links[3])) {
        ret = std::move(std::regex_replace(ret, links[3], "$1"));
    }

    if (std::regex_search(ret, links[4])) {
        ret = std::move(std::regex_replace(ret, links[4], ""));
    }

    if (std::regex_search(ret, links[5])) {
        ret = std::move(std::regex_replace(ret, links[5], ""));
    }

    if (std::regex_search(ret, links[6])) {
        ret = std::move(std::regex_replace(ret, links[6], ""));
    }
...
int main(int argc, char** argv)
{
    ...
    while (itr != fundMap.end()) {
        std::cout << itr->first << " => " << removeAllMarkups(itr->second) << std::endl;
        ++itr;
    }
