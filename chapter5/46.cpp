...
public:
    PredicateParticle(const std::string& i_verb, const std::vector<std::string>& i_particles, const std::vector<std::string>& i_sentenses)
      : verb(i_verb)
      , particles(i_particles)
      , sentenses(i_sentenses)
    {
    }
    ...
    const std::string toString() const
    {
        ...
        for (auto itr = this->sentenses.begin(); itr != this->sentenses.end(); ++itr) {
            ss << *itr;
            if (itr != this->sentenses.end()) {
                ss << '\t';
            }
        }
        return ss.str();
...
public:
    ...
    void addPredicateParticle(const std::string& verb, const std::vector<std::string>& particles, const std::vector<std::string>& clauses)
    {
        this->predicateParticles.push_back(PredicateParticle(verb, particles, clauses));
    }
    ...
    for (const Sentense& sentense : sentenses) {
        for (const Chunk& chunk : sentense) {
            ...
            for (int src : chunk.getSrcs()) {
                ...
                clauses.push_back(sentense[src].getClause());
            }
